...
- **`TAB` for fold.** Same pattern as edamagit-evil: bind `TAB` → `extension.vim_tab` only outside our buffers (`editorLangId != edamajutsu`), remove the global `extension.vim_tab` binding, and edamajutsu's default `tab` → `editor.toggleFold` binding takes over inside edamajutsu.
